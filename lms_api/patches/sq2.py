...
# bench --site all execute lms_api.patches.sq2.execute_all
def execute_all(multi=False):
    # stop_time = datetime.datetime.now() + datetime.timedelta(minutes=4)


    #get quizzes with score 0


    zeros = frappe.db.sql("""select name from `tabQuiz Activity` where score=0 and DATE(creation) >= '2020-10-13'""")

    for z in zeros:

        print(z)
        qa = frappe.get_doc("Quiz Activity",z[0])

        if len(qa.result) != 0:
            continue

        all = frappe.db.sql("SELECT * FROM `tabScheduled Quizzes` where data like '%{0}%'".format(z[0]),as_dict=1)
...
def evaluate(student_email, quiz_response, quiz_name, course, program,activity_name):
    ...
    if student:
        ...
        print("done eval")
        #result = answer
        add_quiz_activity(quiz_name, quiz_response, result, score, status,activity_name)
...
def add_quiz_activity(quiz_name, quiz_response, answers, score, status, activity_name):
    # try:
    result = {}
