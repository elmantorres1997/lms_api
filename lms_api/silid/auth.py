...
class LoginManager:
	...
	def post_login(self):
		...
		self.set_user_info()
		enqueue_send()
		# self.notify_user_sms()

	# def notify_user_sms(self):
	# 	user_info = frappe.db.sql(f"SELECT first_name,last_name, student_mobile_number FROM tabStudent WHERE student_email_id='{frappe.session.user}'",as_dict=1)
...
def enqueue_send():
	...
	enqueue("frappe.auth.send_to_memo", queue='long', is_async=False)
	...
	if wela_setting.text_notification == 1:
		if wela_setting.text_notification_service == "Wela":
			try:
				...
				wela_api_password = get_decrypted_password("Wela Settings", "Wela Settings",fieldname="wela_password")
				conn = FrappeClient(wela_setting.wela_url, wela_setting.wela_username, wela_api_password)
			except:
