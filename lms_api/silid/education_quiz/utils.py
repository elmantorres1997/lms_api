...
def validate_overlap_for(doc, doctype, fieldname, value=None):
    ...
    if existing:
        frappe.throw(_("This {0} conflicts with {1} for {2} {3}").format(doc.doctype, existing.name,
                                                                         ...
                                                                         value or doc.get(fieldname)), OverlapError)
...
def get_portal_programs():
    ...
    portal_programs = [{'program': program, 'has_access': allowed_program_access(program.name)} for program in
                       program_list if allowed_program_access(program.name) or program.allow_self_enroll]
...
def add_activity(course, content_type, content, program):
    ...
    student = get_current_student()
    if not student:
        return frappe.throw(_("Student with email {0} does not exist".format(frappe.session.user)),
...
def evaluate_quiz(quiz_response, quiz_name, course, program):
    # enqueue("erpnext.education.utils.evaluate_quiz_enqueue", queue='long', quiz_response=quiz_response, quiz_name=quiz_name, course=course, program=program)
    # enqueue("erpnext.education.utils.schedule_quiz_evaluation", queue='long', quiz_response=quiz_response, quiz_name=quiz_name, course=course, program=program)
    schedule_quiz_evaluation(quiz_response, quiz_name, course, program)
    return {'result': [], 'score': 1, 'status': "Done"}

@frappe.whitelist()
def schedule_quiz_evaluation(quiz_response, quiz_name, course, program):
    student = get_current_student()
    enrollment = get_or_create_course_enrollment(course, program)
    quiz_activity_name = pre_quiz_activity(enrollment, quiz_name, quiz_response)
    if student:
        data = {
...
@frappe.whitelist()
def pre_quiz_activity(self, quiz_name, quiz_response):
    quiz_info = None
    try:
    ...
    quiz_response_plot = []
    item = {}
    for quiz_res in quiz_response:
        item.clear()
        item = {}
        answer = f"{quiz_response[quiz_res]}".replace('[','').replace(']','').replace("None", "no answer").replace("'","").replace('"',"")
        item['question'] = quiz_res
        item['answer']=  answer or "no answer"
        item['result_silid'] = ""
        item['quiz_result'] = "Wrong"
        item['evaluation_result'] = ""
        item['selected_option'] = "initial"
        quiz_response_plot.append(item.copy())
...
@frappe.whitelist()
def evaluate_quiz_enqueue(quiz_response, quiz_name, course, program):
    import json
    try:
        student = get_current_student()
...
def get_quiz(quiz_name, course):
    ...
    for question in questions:
        for option in question.options:
            print(option.name,option.option,option.image)
    ...
    questions = [{
                     ...
                     'options': [{'name': option.name, 'option': option.option,'image':option.image}
                                 for option in question.options],
                 } for question in questions]

    if has_super_access():
        return {'questions': questions, 'activity': None,'is_shuffle':quiz.shuffle_question}

    student = get_current_student()
    course_enrollment = get_enrollment("course", course, student.name)
    status, score, result = check_quiz_completion_old(quiz, course_enrollment)
    return {'publish_date': quiz.get_publish_date(), 'questions': questions,'is_shuffle':quiz.shuffle_question, 'activity': {'is_complete': status, 'score': score, 'result': result}}
...
def get_or_create_course_enrollment(course, program):
    ...
    if not course_enrollment:
        ...
        if not program_enrollment:
            ...
            return
        return student.enroll_in_course(course_name=course,
                                        program_enrollment=get_enrollment('program', program, student.name))
    else:
...
def check_content_completion(content_name, content_type, enrollment_name):
    activity = frappe.get_all("Course Activity", filters={'enrollment': enrollment_name, 'content_type': content_type,
...
def check_quiz_completion(quiz, enrollment_name):
    attempts = frappe.get_all("Quiz Activity", filters={'enrollment': enrollment_name, 'quiz': quiz.name},
                              fields=["name", "activity_date", "score", "status"])
...
def check_quiz_completion_old(quiz, enrollment_name):
    attempts = frappe.get_all("Quiz Activity", filters={'enrollment': enrollment_name, 'quiz': quiz.name},
                              fields=["name", "activity_date", "score", "status"])
    status = False if quiz.max_attempts == 0 else bool(len(attempts) >= quiz.max_attempts)
