...
def get_modules_from_all_apps_for_user(user=None):
	...
	try:
		allow_lms = ["LMS User", "Instructor", "Student"]
		if bool(set(allow_lms).intersection(roles)):
			if "lms" in blocked_modules:
				blocked_modules.remove("lms")

		# blocked_modules.append("pending-tasks")

		if "Social" in blocked_modules:
			blocked_modules.remove("Social")
		if "help" in blocked_modules:
			blocked_modules.remove("help")
		if "Written Activity" in blocked_modules:
			blocked_modules.remove("Written Activity")

		if "Class" in blocked_modules:
			blocked_modules.remove("Class")

		if "attendance-sheet" in blocked_modules:
			blocked_modules.remove("attendance-sheet")
		if "master-grade" in blocked_modules:
			blocked_modules.remove("master-grade")

		# Remove for student
		if "Student" == roles[0]:
			blocked_modules.append("attendance-sheet")
			blocked_modules.append("master-grade")
			# blocked_modules.remove("pending-tasks")
