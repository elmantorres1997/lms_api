...
from frappe.service import global_var
...
def is_direct(owner, other, bidirectional = False):
	def get_room(owner, other):
		filters = {"`tabChat Room User`.owner": owner, "user": other, "type":"Direct"}
		room = execute_query("GET_CHATROOM", filters=filters)
		return room

	exists = len(get_room(owner, other)) > 0
	if bidirectional:
		exists = exists or len(get_room(other, owner)) > 0
	print("DOES ROOM EXISTS:", exists)
	return exists
	...
	if not fields or 'last_message' in fields:
		for i, r in enumerate(rooms):
			...
			if droom.last_message:
				...
				# if frappe.session.user not in rooms[i]['last_message']['seen']:
					# notification_count+=1
			else:
				rooms[i]['last_message'] = None

	rooms = squashify(dictify(rooms))
	# if global_var.get_stored_csrf(frappe.session.user) != frappe.session.data.csrf_token:
	# 	if int(notification_count):
	# 		frappe.msgprint(f"You have {notification_count} unread messages.")
	# 	global_var.change_store_csrf(frappe.session.user, frappe.session.data.csrf_token)
	return rooms
...
def create(kind, owner, users = None, name = None):
	...
	if kind == "Direct":
		...
		if not is_direct(owner, other, bidirectional=True):
			room.save(ignore_permissions = True)
		else:
			frappe.throw(_('Direct room with {0} already exists.').format(other))
