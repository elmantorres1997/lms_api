<template>
	<div class="post-card">
		<div class="post-body" :class="{'pinned ': is_pinned}">
			...
			<div ref="content" class="content course-badge course text-muted" v-if="post.course">Course: {{ post.course }}</div>
		</div>
		<post-action
...
export default {
	...
	beforeMount(){
		frappe.db.count("Post Comment", {filters:{ parent: this.post.name }}).then(response =>{
	...
	methods: {
		...
		delete_post() {
			frappe.confirm(__("Are you sure you want to delete this post?"), () => {
                ...
                this.$root.$emit("delete_post", (this.post.name));
			})
