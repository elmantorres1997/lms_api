...
frappe.chat.room.create = function (kind, owner, users, name, fn) {
    ...
    $(".frappe-chat-room-list").css("pointer-events", "none");
    return new Promise(resolve => {
        frappe.call({
            method: "frappe.chat.doctype.chat_room.chat_room.create",
            args: { kind: kind, owner: owner || frappe.session.user, users: users, name: name },
            btn:$('.row'),
            freeze:true,
            callback: (r) => {
               $(".frappe-chat-room-list").css("pointer-events", "auto");
                let room = r.message
...
frappe.chat.message.on.create = function (fn) {
	frappe.realtime.on("frappe.chat.message:create", r =>
		fn({
		...r, creation: new frappe.datetime.datetime(r.creation),
		content: r.content.path ? h("a", { class: "no-decoration", href: r.content.path, target: "_blank" },
                                h(frappe.components.FontAwesome, { type: "file", fixed: true }), ` ${r.content.path.replace(/^.*[\\\/]/, '')}`
								) : r.content
		})
	)
	frappe.realtime.on("frappe.chat.message:create", data =>{
        if (frappe.session.user !== data.user) {
            notify(data.content, data.user,true);
        }
	})
}
		...
		return (
			h("div",{class:`chat-bubble ${props.groupable ? "chat-groupable" : ""} chat-bubble-${me ? "r" : "l"}`,
				...
				h("div",{class:"chat-bubble-content"},
						h("small","",
							(props.type === "File") || (content.path) ?
								h("a", { class: "no-decoration", href: content.path, target: "_blank" },
									h(frappe.components.FontAwesome, { type: "file", fixed: true }), ` ${content.path.replace(/^.*[\\\/]/, '')}`
								)
								:
								content
						)
